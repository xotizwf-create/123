...
from backend.models import OTPChallenge, Profile, TelegramPending, User
...
def create_app() -> Flask:
    ...
    def _rate_limit_otp(db_session, user_id: int, channel: str, purpose: str) -> str | None:
        ...
        if recent and (now - recent[0].created_at).total_seconds() < 60:
            return "OTP was sent recently. Please wait a minute."
    ...
    def _consume_otp(db_session, challenge: OTPChallenge, code: str) -> bool:
        ...
        if challenge.attempts >= 5:
            return False
        if not _verify_otp_hash(code, challenge.code_hash):
    ...
    def profile():
        ...
        try:
            ...
            if not user.profile:
                ...
                commit_with_retry(db_session)
            if request.method == "POST":
                user.profile.full_name = (request.form.get("full_name") or "").strip()
